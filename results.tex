...
-- Standard GCC levels do not always optimize for resource consumption.\\
...
\paragraph{Results}
Figure 8 visualizes the Pareto optimal solutions generated that achieved the best performance assessment for the objective \textit{$<$ExecutionTime--MemoryUsage$>$}. The horizontal axis indicates the memory usage in raw data (in Bytes) as it is collected using NOTICE. In similar fashion, the vertical axis shows the execution time in seconds. Furthermore, the figure shows the impact of standard GCC options and best NS sequences for memory and execution time. Based on these results, we can see that NSGA-II outperforms better than NS-II. In fact, these best solutions from the Pareto-optimal front leads to best trade-off between the two objectives. Therefore, it is up to compiler users to use one solution from this Pareto front (6 solutions for NSGA-II and 5 for NS-II) that satisfies his needs. For example, he could choose one solution that maximize the execution speed in favor of memory reduction. On the other side, NS-II is capable to generate only one solution that is non-dominated by any other one. 
...
-- A multi-objective approach is useful to build compiler optimizations that present a trade-off between memory usage and execution time. \\
