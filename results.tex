...
\begin{table}[h]
	\centering
	\caption{Results of mono-objective optimization}
	\label{my-label}
...
\paragraph{Method}
Another interesting experiment is to test the sensitivity of input programs to compiler optimizations and evaluate the general applicability of best optimal optimization sets, previously discovered in RQ1. Theses sequences correspond to best sequences generated using NS for the 3 non-functional properties that we are focusing on namely S, MR and CR. 
Thus, we apply best discovered optimizations to new unseen Csmith (100 new random programs) and we compare then, the performance improvement across these programs (see right-hand side of figure 5). We also apply standard optimizations, O2 and O3, to new Csmith programs to compare performance results.
The idea of this experiment is to test whether new Csmith programs are sensitive to previously discovered optimizations or not. If so, this will be useful for compiler users and researchers to use NOTICE in order to build general optimization sequences from their representative \textit{training set} programs.

\paragraph{Results}
Figure 6 shows the distribution of memory, CPU and speedup improvement across new Csmith programs. For each non-functional property, we apply O2, O3 and best NS sequences. Speedup results show that the 3 optimization strategies have almost the same distribution with a median value of 1.12 for speedup. This can be explained by the fact that NS needs too long time to find the sequence that will better optimize the execution speed. O2 and O3 have also the same impact on CR and MR which is almost the same for both levels (CR median value is 8\% and around 5\% for MR).
However, the impact of applying best generated sequences using NS clearly outperform O2 and O3 with almost 10\% of CPU improvement and 7\% of memory improvement. This proves that NS sequences are efficient and can be used to optimize resource consumption of Csmith programs.
...
%\end{figure}
Figure 6 shows the impact of speedup optimization on resource consumption. It is important to notice that there is no correlation between speedup and resource usage. For instance, O2 and O3 that have the best speedup improvement present opposite impact on resource usage. Applying O2 induces -8.4\% of MR and -5\% of CR. However, applying O3 improve MR and CR by 4.2\% and 3.4\%. The outcome of applying different mono-objective algorithm for optimizing speedup also shows that resource consumption is always in conflict with execution speed. The highest CR and MR improvement is reached using NS with only 1.2\% and 5.4\%. This improvement is considerably low compared to scores reached when we used resource usage metrics as key objectives in RQ1 (i.e., 15.6\% for MR and 23.5\% for CR). These results agree to the idea that compiler optimizations do not put too much emphasis on the trade-offs execution time and resource consumption.
...
-- There is no consensus between application performance and resource consumption.\\
-- Applying standard compiler levels can induce undesirable effects on system resources }}
