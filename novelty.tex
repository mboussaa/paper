...
In fact, since the search space of possible combinations is too large, we aim at using a new search-based technique called Novelty Search to tackle this issue. 
The idea of this approach is to explore the search space of possible compiler flag options without regard to any objective. 
Instead of having a fitness-based selection that aim at optimizing either the execution time or the energy consumption, we select optimization sequences based on a novelty score showing how different they are compared to all other combinations evaluated so far. 
This makes the tool fitness and program independent.
...
For instance, in a previous work Chen et al.~\cite{chen2012deconstructing} showed that handful optimizations may lead to higher speedup than other techniques of iterative optimization. 
...
However, all these efforts use a fitness-based selection to guide the search. 
...
Therefore, during the evolutionary process, we select optimization sequences that remain in sparse regions of the search space in order to guide the search through novelty. 
In the meanwhile, we choose to gather non-functional metrics of explored sequences such as memory consumption. 
These metrics will provide us a more fine-grained understanding and analysis of compiler's behavior regarding optimizations. 

\begin{algorithm}
	\caption{Novelty search algorithm for compiler optimizations exploration}
	\label{alg1}
	\begin{algorithmic}[1]
		\REQUIRE Optimization sequences $\mathcal{S}$
		\REQUIRE Benchmark programs $\mathcal{B}$
		...
		\STATE $initialize\_archive(archive,\mathcal{L})$
		\STATE $population \gets random\_sequences(\mathcal{S})$
		\REPEAT
		
		\FOR{$sequence \in population$}   
		\FOR{$program \in \mathcal{B}$}
...
This measure expresses the optimization sequence unicity. 
We can quantify the sparseness of a solution as the average distance to the k-nearest neighbors. If the average distance to
...
a dense region then it will get a low novelty score. Algorithm 1 shows an overview of the evolutionary process of NS. 
