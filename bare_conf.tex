...
\title{A Novelty Search approach for automatic Test Data Generation}
...
Olivier Barais\IEEEauthorrefmark{1},
Gerson Sunye\IEEEauthorrefmark{1},
and Benoit Baydry\IEEEauthorrefmark{1} }
\IEEEauthorblockA{\IEEEauthorrefmark{1}Inria / IRISA, University of Rennes 1, France\\
	...
	\IEEEauthorrefmark{2}olivier.barais@inria.fr,
	\IEEEauthorrefmark{3}gerson.sunye@inria.fr,
	\IEEEauthorrefmark{4}benoit.baudry@inria.fr}	
}
...
\begin{abstract}
%\boldmath
In search-based structural testing, meta-heuristic search techniques have been frequently used to automate test data generation. In Genetic Algorithms (GA) for example, test data are rewarded on the basis of an objective function that represents the number of statements executed. However, owing to the wide diversity of possible test data values, it is hard to find the set of test data that could satisfy a specific coverage criterion. In this paper, we propose, for the first time, the use of novelty search algorithm to the test data generation problem based on statement-covered criteria. We believe that such approach to test data generation is attractive because it allows the exploration of the huge space of test data within the input domain. In this approach, we seek to explore the search space without regard to any objectives. In fact, instead of having a fitness-based selection we would like to select test cases based on a novelty score showing how different they are compared to all other solutions evaluated so far. We implemented our Novelty Search algorithm and evaluated it comparing to fitness-based and random testing approaches.
\end{abstract}
...
%%\subsection{Search-based structural testing}
...
\begin{algorithm}
	...
	\DontPrintSemicolon
	\KwIn{Methods interface I }
	\KwIn{Number of iterations N }
	\KwIn{Target classes C }
	\KwIn{Max coverage maxCoverage }
	\KwIn{Threshold T }
	\KwIn{Limit L }
	\KwIn{Nearest neighbors k }
	\KwOut{Set of relevant test cases bestSoltions}
	...
	\Repeat{generation = N}{
		$testCase \gets methodCall (methodName, data) $\;
		$testSuite \gets setOf(testCase) $\;
		$P \gets setOf(testSuite)$\;
		\For{$testSuite \in P$}{
			\For{$testCase \in testSuite$}{
				\textsl{}
				$coveredStatements \gets execute(testCase,C)$\;
			}
			$noveltyMetric \gets distFromkNearest(testSuite,archive,P,k)$\;
			
			\If{$noveltyMetric > T$}{
				$archive \gets archive + testSuite$\;
				$selectedTestSuite \gets selectedTestSuite + testSuite$\;
			}
			
			\If{$coveredStatements \geq  maxCoverage $}{
				$bestSolutions \gets bestSolutions + testSuite$\;
				
			}
		}
		$P \gets generateNewPopulation(selectedTestSuite)$\;
