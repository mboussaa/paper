...
For instance, we count 76 optimization flags that are enabled by 4 default optimization levels (O1, O2, O3, Ofast). 
For instance, O1 reduces code size and execution time without performing any optimizations that take a great deal of compilation time. It turns on 32 flags. 
...
This results in a huge space with $2^{76}$ possible optimization combinations. The list of optimizations is available here~\cite{mboussaa}.
